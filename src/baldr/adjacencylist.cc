...
void AdjacencyList::add(const uint32_t label, const float sortcost) {
  get_bucket(sortcost).push_back(label);
}

// The specified label now has a smaller cost.  Reorders it in the sorted list
void AdjacencyList::decrease(const uint32_t label,
  ...
  auto& newbucket  = get_bucket(newsortcost);
...
std::deque<uint32_t>& AdjacencyList::get_bucket(const float cost) {
...
// low level buckets.
void AdjacencyList::empty_overflow() {
  bool found = false;
  ...
  while (!found && !overflowbucket_.empty()) {
    ...
    while (!overflowbucket_.empty()) {
      ...
      // Get the cost (using the edge cost function)
      float cost = edgecost_(label);
      if (cost < maxcost_) {
